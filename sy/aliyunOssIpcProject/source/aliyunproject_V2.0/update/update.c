...
int reportImageUpdateResult(SERVICEVERSION returnInfo, char *operTime)
{
	...
	sprintf(sendBuf, "ipc_id=%s&result=%d&HwVersion=%s&SwVersion=%s&updateTime=%s",g_szServerNO, returnInfo.result, "", "", operTime);
	...
	while(1)
	{
		...
		if(hour == 1 && isUpdate == 1)
		{
			...
			while(1)
			{
				...
				sprintf(version, "%s-%s", SDK_HARD_FWVERSION, SDK_SYSTEM_FWVERSION);
				LOGOUT("version:%s---\n", version);
				printf("version:%s---\n", version);
