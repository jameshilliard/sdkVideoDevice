...
/*****************************************************************************
  ...
  Called By:
  Input:       
  Output:      config  --pointer to an configuration information structure.
  Return:      
  Others:
*****************************************************************************/
HI_S32 AACInitDefaultConfig(AACENC_CONFIG *config);
HI_S32 AACInitGetDefaultConfig(AACENC_CONFIG *config, AuEncoderFormat enFormat);
HI_S32 AACCheckConfig(AACENC_CONFIG *config);
...
/*****************************************************************************
  ...
  Input:       phAacPlusEnc   --pointer to an encoder handle, initialized on return
               pconfig        --pre-initialized config struct
  Output:      
  Return:      AACENC_OK if success
  Others:
*****************************************************************************/
HI_S32  AACEncoderOpen(AAC_ENCODER_S ** phAacPlusEnc, AACENC_CONFIG *pconfig);
...
/*****************************************************************************
  ...
  Input:       hAacPlusEnc   --encoder handle
               timeSignal    --BLOCKSIZE*nChannels audio samples, interleaved
			   outBytes      --pointer to output buffer 
			                   (must be 6144/8*MAX_CHANNELS bytes large)
               numOutBytes   --number of bytes in output buffer after processing
  Output:      
  Return:      AACENC_OK if success
  Others:
*****************************************************************************/
HI_S32 AACEncoderFrame(AAC_ENCODER_S* hAacPlusEnc, HI_S16 *timeSignal,
				       HI_U8 *outBytes, HI_S32 *numOutBytes);
...
/*****************************************************************************
  ...
  Others:
*****************************************************************************/
HI_VOID AACEncoderClose (struct AAC_ENCODER *hAacPlusEnc);  
