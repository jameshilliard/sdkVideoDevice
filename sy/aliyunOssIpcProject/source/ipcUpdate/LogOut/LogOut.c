...
void Init_LogOut(INT32U v_iLogSize,LPCTSTR v_szFlushPath, BOOL v_bOutToSerial,LPCTSTR v_szTempPath)
{
	...
	for (i = 0; i<MAX_LOGFILE; i++)
	{
		...
		sprintf(szPath, "%s/%s/UPDATELOG%d_00.log", g_szFlushLogPath, LOGFILEDIR, i);
...
void BackupFile(INT32U v_iLv)
{
	...
	{
		sprintf(szNewPath, "%s/%s/UPDATELOG%d_%02d.log", g_szFlushLogPath, LOGFILEDIR, v_iLv, i+1);
		sprintf(szOldPath, "%s/%s/UPDATELOG%d_%02d.log", g_szFlushLogPath, LOGFILEDIR, v_iLv, i);
