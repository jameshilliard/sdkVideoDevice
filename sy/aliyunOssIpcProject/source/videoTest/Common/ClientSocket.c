...
int SendBufferData(int socketId,DWORD v_dwSymb,char *v_pDataBuf,DWORD v_dwDataLen)
{
	...
	for(i=0; i< MAXSOCK; i++)
	{
		if(socketInfo[i].socketId == socketId)	
		{
			if(socketInfo[i].pIInnerDataList)
			{
				iRet=socketInfo[i].pIInnerDataList->pushData(socketInfo[i].pIInnerDataList->_this,
				v_bytLvl,v_dwDataLen,v_pDataBuf,v_dwSymb,&v_dwPacketNum);
			}
			break;
		}
	}
